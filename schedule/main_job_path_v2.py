...
from app_vdp_wric_user_path_data_v2 import DATATREND  # path_data表
from app_vdp_wric_user_path_keyword_v2 import KEYWORD  # 关键字日表
...
def run_path_daily(date_list):
    ...
    bd_id = date_list[1]
    ht = HiveTask()
    datatrend = DATATREND(jobdate, ht, bd_id)
    datatrend.run()
    print("PATH finished!")
...
def run_keyword_daily(date_list):
    jobdate = date_list[0]
    ht = HiveTask()
    keyword_day = KEYWORD(jobdate, ht)
    keyword_day.run()
...
class PATH_MAIN(threading.Thread):
    def __init__(self, end_date, ht, pool,bd_id=None):
        super(PATH_MAIN, self).__init__()
        print ("shopping_path购物路径开始运行...")
        self.ht = ht
