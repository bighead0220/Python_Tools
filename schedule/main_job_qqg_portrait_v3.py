...
from app_vdp_wric_qqg_user_sales_portrait_v3 import QQGPortrait  # portrait表
from app_vdp_wric_qqg_user_sales_portrait_all_v3 import QQGPORTRAITALL  # 品类画像表-由上一个临时表脱去用户维度汇总而来
from app_vdp_wric_qqg_user_sales_portrait_compare_v3 import QQGPORTRAITCOMPARE  # 品牌商下品牌用户画像表,同比表compare
from app_vdp_wric_qqg_user_sales_portrait_ration_v3 import QQGPORTRAITRATION  # 品牌商下品牌用户画像表,画像各属性比例表ration
from app_vdp_wric_qqg_user_sales_portrait_all_compare_v3 import QQGALLCOMPARE  # 全品类下用户画像表,同比表all_compare
from app_vdp_wric_qqg_user_sales_portrait_all_ration_v3 import QQGALLRATION  # 全品类下用户画像表,画像各属性比例表all_ration
from app_vdp_wric_qqg_user_sales_portrait_pre_v3 import QQGPortraitPre
from app_vdp_wric_qqg_user_sales_portrait_all_compare_plus_v3 import QQGALLCOMPAREPLUS
from app_vdp_wric_qqg_user_sales_portrait_all_ration_plus_v3 import QQGALLRATIONPLUS
from app_vdp_wric_qqg_user_sales_portrait_compare_plus_v3 import QQGPORTRAITCOMPAREPLUS
from app_vdp_wric_qqg_user_sales_portrait_ration_plus_v3 import QQGPORTRAITRATIONPLUS
from app_vdp_wric_qqg_user_sales_portrait_kanban_plus_v3 import QQGKANBANPLUS
from app_vdp_wric_qqg_user_sales_plus_user_v3 import QQGPLUSUSER
from app_vdp_wric_qqg_user_sales_plus_amt_v3 import QQGPLUSAMT
from app_vdp_wric_qqg_user_sales_plus_tb_v3 import QQGPLUSTB
...
def run_portrait_batch_base(date_list):
    ...
    portraitpre = QQGPortraitPre(jobdate, ht, bd_id)
    portraitpre.run()  #并发run_portrait_batch_base主函数时, 先并发run()函数,等run()的跑完1个,才会调起portall和portrait,且由于这两个是start并发,会同时调起run()刚结束的那个日期
    portraitall = QQGPORTRAITALL(jobdate, ht, bd_id)
    portraitall.start()
    portrait = QQGPortrait(jobdate, ht, bd_id)
    portrait.start()
    plusamt = QQGPLUSAMT(jobdate, ht, bd_id)
    plusamt.start()
    # portrait结束
    portrait.join()
    # portrail_all结束
...
def run_portrait_batch_cur(date_list_cur_year):
    ...
    ht = HiveTask()
    # portrait后续开始
    portraitcompare = QQGPORTRAITCOMPARE(jobdate, ht, bd_id)
    portraitration = QQGPORTRAITRATION(jobdate, ht, bd_id)
    portraitcompare.start()
    portraitration.start()
    portraitcompareplus = QQGPORTRAITCOMPAREPLUS(jobdate, ht, bd_id)
    portraitrationplus = QQGPORTRAITRATIONPLUS(jobdate, ht, bd_id)
    portraitcompareplus.start()
    portraitrationplus.start()
    kanbanplus = QQGKANBANPLUS(jobdate, ht, bd_id)
    kanbanplus.start()
    # portrait_all后续开始
    allcompare = QQGALLCOMPARE(jobdate, ht, bd_id)
    allration = QQGALLRATION(jobdate, ht, bd_id)
    allcompareplus = QQGALLCOMPAREPLUS(jobdate, ht, bd_id)
    allrationplus = QQGALLRATIONPLUS(jobdate, ht, bd_id)
...
class PORTRAIT_MAIN_QQG(threading.Thread):
    def __init__(self, start_dt,end_dt,pool, ht, bd_id=None):
        super(PORTRAIT_MAIN_QQG, self).__init__()
        print("user_por全球购用户细分开始运行...")
        self.ht = ht
        self.bd_id = bd_id
        self.pool = int(pool)
...
def main():
    ...
    if len(sys.argv) == 4:
        ...
        pool = sys.argv[3]
        bd_id = None
    main_job = PORTRAIT_MAIN_QQG(start_dt,end_dt, pool,ht, bd_id)
    main_job.run()
